...
class BaseModel():
    ...
    def print_network(self, net):
        ...
        net_params_trainable = sum(p.numel() for p in net.parameters() if p.requires_grad)

        logger.info(
            f'Network: {net_cls_str}, with parameters: {net_params:,d}, trainable: {net_params_trainable:,d}')
        logger.info(net_str)
    ...
    def save_network(self, net, net_label, current_iter, param_key='params'):
        ...
        for net_, param_key_ in zip(net, param_key):
            ...
            ###
            if hasattr(net_ ,'hq_clip_feature'):
                logger.info(f'Saving hq_clip_feature to state_dict.')
                state_dict['hq_clip_feature']= net_ .hq_clip_feature
    ...
    def load_network(self, net, load_path, strict=True, param_key='params'):
        ...
        ### 暂存hq_clip_feature,加载完成之后赋给net
        # hq_clip_feature = load_net['hq_clip_feature']
        hq_clip_feature = load_net.pop('hq_clip_feature') if 'hq_clip_feature' in load_net else None
        ...
        logger.info(f'Loading hq_clip_feature.')
        if hq_clip_feature is not None:
            net.hq_clip_feature.data = net.hq_clip_feature.data.repeat(hq_clip_feature.shape[0], 1)
            net.hq_clip_feature.data.copy_(hq_clip_feature)
            logger.info(f'{ net.hq_clip_feature}')
        else:
            logger.info(f'hq_clip_feature is {hq_clip_feature}, w/o visual ablation.')
...
class BaseModel_Restormer():
    """Base model. 不保存hq-clip-emb的"""
